...
#Snowy death
func _on_northern_death_body_entered(body):
	if body.name == "Player":
...
func _on_northern_attack_body_entered(body):
	if body.name == "Player":
		# Start chasing the player
		#chase = true
		start_attack()
...
func _on_northern_attack_body_exited(body):
	if body.name == "Player":
		#chase = false
		stop_attack()
		if health <= 0:
...
func dealDamageToPlayer():
	if player:
		...
		Game.playerHP -= 2 # Adjust the amount of damage as needed
